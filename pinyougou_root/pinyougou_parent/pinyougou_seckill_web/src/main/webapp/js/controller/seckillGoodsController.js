...
app.controller('seckillGoodsController', function ($scope, $location, seckillGoodsService, $interval) {
    ...
    let convertTimeString = function (allsecond) {
        let days = Math.floor(allsecond / (60 * 60 * 24));//天数
        let hours = Math.floor((allsecond - days * 60 * 60 * 24) / (60 * 60));//小时数
        let minutes = Math.floor((allsecond - days * 60 * 60 * 24 - hours * 60 * 60) / 60);//分钟数
        let seconds = allsecond - days * 60 * 60 * 24 - hours * 60 * 60 - minutes * 60; //秒数
        let timeString = "";
        if (days > 0) {
            timeString = days + "天 ";
        }
